...
- `backend/api/scrapers/registry.py` is the single registry for enabled search sources and detail-capable sources. Search obtains the fixed board adapters plus one adapter per validated company-catalog target from this registry, while the detail route resolves a source by its canonical `source_name` and accepts only scrapers implementing the `DetailJobScraper` protocol. Cached factories and the cached company scraper tuple ensure every backend process reuses each scraper and its shared rate limiter.
...
- Incomplete summaries carry `details_loaded=false`. Selecting one calls `GET /jobs/details/{source_id}/{external_id}`, lets the registry delegate to that source's detail-capable scraper, replaces the summary in both visible results and the local cache, and enables star/applied actions only after the complete snapshot arrives. SwissDevJobs and company ATS entries are already complete and skip this request.
...
- `POST /jobs/scrape` runs jobs.ch, jobup.ch, SwissDevJobs, and one independent source per catalogued company through the same bounded source executor and returns normalized jobs directly. Search orchestration performs no DynamoDB writes; source counts and failures go through Python logging to CloudWatch in Lambda.
...
## Company ATS Scraping (Greenhouse and Lever)

- `backend/api/scrapers/company_targets.json` is the validated, versioned catalog of company career sources. It currently maps Scandit and On to Greenhouse and RIVR and SwissBorg to Lever. `backend/api/scrapers/ats/targets.py` validates each stable ID, company name, careers URL, ATS discriminator, and provider token before network work begins.
- `backend/api/scrapers/registry.py` expands every catalog entry into a separate cached source named `company:<id>`. Company requests use the existing source executor, HTTP client, retry policy, and per-scraper rate limiter, so one unavailable company produces its own failed source result without stopping boards or other companies.
- `backend/api/scrapers/ats/greenhouse.py` requests the public Greenhouse board API once with `content=true`. It removes non-vacancy prospect posts whose `internal_job_id` is null, rejects malformed or wholly unusable payloads, deduplicates IDs, converts HTML content to a full normalized description, and returns complete jobs.
- `backend/api/scrapers/ats/lever.py` supports both global and EU public Lever posting APIs. It requests up to five pages of 100 jobs with `skip`/`limit`, folds every labelled `lists` section into the searchable description, maps common requirement-style headings separately, and deduplicates overlapping IDs. One extra empty-page probe accepts an exactly full final page, while a repeated page, malformed payload, or a genuinely exceeded cap fails visibly.
- `backend/api/scrapers/ats/search.py` applies the existing live-search term and location to complete ATS jobs locally. Matching is case- and accent-insensitive across normalized job fields, so the company APIs do not need provider-specific query behavior.
- `backend/api/services/swiss_territory.py` gives a structured ATS country value precedence and otherwise recognizes only reviewed Swiss country, canton, and employment-centre names. The central orchestration filter then retains only exact `CH` results; foreign and unclassified jobs from global company boards are omitted.
- ATS jobs set `details_loaded=true` and go directly through the existing response, deduplication, browser-cache, and interaction flows. Their `raw_payload` keeps only a parser name and stable target ID; full content is retained once in normalized fields instead of repeated in synchronous Lambda responses and saved snapshots.
- `backend/tests/test_company_targets.py`, `backend/tests/test_greenhouse_scraper.py`, `backend/tests/test_lever_scraper.py`, `backend/tests/test_swiss_territory.py`, and `backend/tests/test_scraper_registry.py` use mocked payloads to cover catalog validation, API URLs, positive and negative local filters, arbitrary Lever list headings, Swiss evidence, pagination boundaries and guards, deduplication, and registry expansion without live traffic.
...
- Greenhouse and Lever company boards can contain worldwide vacancies. Their adapters prefer a structured country value and otherwise use the conservative Swiss territory helper; an unknown place stays unclassified instead of being guessed from the company's nationality.
...
- Scraper parser and Swiss-territory tests cover evidence-backed country assignment, while `backend/tests/test_scrape_orchestration.py` covers `CH`, foreign, and missing country codes at the central gate.
