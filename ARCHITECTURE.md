...
- `backend/scripts/cleanup_legacy_jobs.py` scans for old `JOB`, `SOURCE`, and `SCRAPE_RUN` records. It is a dry run unless `--confirm` is passed and never deletes user or interaction items.
